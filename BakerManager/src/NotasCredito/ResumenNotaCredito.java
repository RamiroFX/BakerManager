...
public class ResumenNotaCredito extends JDialog implements ActionListener, KeyListener, InterfaceFacturaDetalle {
    ...
    public void inicializarDatos(List<E_NotaCreditoCabecera> listaNotaCredito){        
        inicializarVista(listaNotaCredito);
    }
        ...
        switch (n) {
            case 0: {
                EventQueue.invokeLater(new Runnable() {
                    @Override
                    public void run() {
                        FacturaCabeceraTableModel tmCabecera = (FacturaCabeceraTableModel) jtCabecera.getModel();
                        ...
                        if (tmCabecera.getFacturaCabeceraList().isEmpty()) {
                            ...
                            facturaCabecera.setTiempo(new Timestamp(cabecera.getTiempo().getTime()));
